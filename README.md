...
uv run pyang -p temp/yang_models/ \
    ...
    temp/yang_models/*.yang > temp/schema.json
```
...
```bash
curl -k -u admin:pass -H "Accept: application/yang-data+json" "https://172.16.42.123:8443/restconf/data?content=config" > temp/instance.json
```
...
```bash
uv run check-jsonschema --check-metaschema temp/schema.json
uv run check-jsonschema --schemafile temp/schema.json temp/instance.json
```

### 5. Pydantic Model Generation

We target **Pydantic V2** using `datamodel-codegen`. Other options are available, see tool's documentation.

> [!INFO]  
> These commands can take several minutes to run on deeply nested models.
